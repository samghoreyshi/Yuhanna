...
export function ToastProvider({ children }) {
  ...
  return (
    <ToastContext.Provider value={{ addToast, removeToast }}>
      ...
      {mounted &&
        createPortal(
          <div className="fixed top-4 left-4 right-4 z-50 flex flex-col items-end gap-2">
            <AnimatePresence>
              {toasts.map((toast) => (
...
function Toast({ id, message, type = 'info', onClose }) {
  ...
  return (
    <motion.div
      ...
      className={`min-w-[320px] max-w-md rounded-xl shadow-lg shadow-black/5 ${className} backdrop-blur-sm`}
    >
      <div className="flex items-start gap-3 p-4">
        <span className="flex-shrink-0">{icon}</span>
        <p className="flex-1 text-right font-IranSans">{message}</p>
        <button
          onClick={onClose}
